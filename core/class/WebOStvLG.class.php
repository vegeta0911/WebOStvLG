...
class WebOStvLG extends eqLogic {
    ...
    if($lgtvscanin == ''){
        ...
        $search = ['https','3001'];
        $replace = ['http','3000'];
        $modifiedJsonData = str_replace($search, $replace, $lgtvjsonInput);
        $result = file_put_contents(self::LG_PATH.'/3rdparty/inputs.json', $modifiedJsonData);
        $lgtvjsonImg = file_get_contents(self::LG_PATH.'/3rdparty/inputs.json');
        $lgtvjsonImage = json_decode($lgtvjsonImg, true);
        //log::add('WebOStvLG', 'debug', '|  json: listInputsImage ' .$lgtvjsonImg );

		foreach ($lgtvjsonImage["payload"]["devices"] as $inputs) {
    ...
    public static function etattv() {
        ...
        {   
          ...
          if($eqLogic->getConfiguration('statut') == 1){
            ...
            $cmd = 'timeout 10 '.system::getCmdSudo().' '.self::EXEC_LG.' --name "'.$lgtvscanin["list"][0]["tv_name"].'" --ssl getPowerState';
...
class WebOStvLGCmd extends cmd {
    ...
    public function execute($_options = null) {
		...
		$ret = shell_exec(system::getCmdSudo().' '.__DIR__ . '/../../resources/venv/bin/python3 /var/www/html/plugins/WebOStvLG/resources/venv/bin/lgtv ' .$command .' '.$message);
                ...
                if($command == $mac){
                    $ret = shell_exec(system::getCmdSudo().' '.__DIR__ . '/../../resources/venv/bin/wakeonlan ' .$command);
                    log::add('WebOStvLG','debug','$$$ EXEC: '.__DIR__ . '/../../resources/venv/bin/wakeonlan ' .$command .' > ' .$ret );
                }
                else
                {
